...
namespace ProgrammingAssignment4
{
    ...
    public class TeddyBear
    {
        ...
        public void Update(GameTime gameTime, MouseState mouse)
        {
            ...
            if (collecting)
            {               
                location.X += (int)(velocity.X * BaseSpeed * gameTime.ElapsedGameTime.Milliseconds);
                location.Y += (int)(velocity.Y * BaseSpeed * gameTime.ElapsedGameTime.Milliseconds);
        ...
        public void SetTarget(Vector2 target)
        {
            ...
            float diffX = target.X - drawRectangle.X - halfDrawRectangleWidth;
            float diffY = target.Y - drawRectangle.Y - halfDrawRectangleHeight;

            double angle = Math.Atan2(-diffY, diffX);

            velocity.X = (float)(Math.Cos(angle));
            velocity.Y = (float)(Math.Sin(angle)) * -1;
        }
