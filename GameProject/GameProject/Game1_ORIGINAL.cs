    ...
    public class Game1 : Game
    {
        ...
        protected override void Update(GameTime gameTime)
        {
                ...
                for (int j = i + 1; j < bears.Count; j++)
                {
                    if (bears[i].Active && bears[j].Active)
                    {
                        ...
                        if (bearsCollision != null)
                        {
                            ...
                            {
                                bears[i].Velocity = bearsCollision.FirstVelocity;
                                bears[i].DrawRectangle = bearsCollision.FirstDrawRectangle;
                            }
                            if (bearsCollision.SecondOutOfBounds)
                            {
                                bears[j].Active = false;
                            }
                            else
                            {
                                bears[j].Velocity = bearsCollision.SecondVelocity;
                                bears[j].DrawRectangle = bearsCollision.SecondDrawRectangle;
                            }
                        }
                    }
                }
            ...
            foreach (TeddyBear bear in bears)
            {
                foreach (Projectile projectile in projectiles)
                {
                    if (projectile.Type == ProjectileType.FrenchFries && projectile.Active && bear.Active)
                    {
